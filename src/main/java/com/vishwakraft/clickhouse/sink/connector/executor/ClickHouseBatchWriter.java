...
import com.vishwakraft.clickhouse.sink.connector.db.BaseDbWriter;
import com.vishwakraft.clickhouse.sink.connector.db.DBMetadata;
import com.vishwakraft.clickhouse.sink.connector.db.DbKafkaOffsetWriter;
import com.vishwakraft.clickhouse.sink.connector.db.DbWriter;
...
public class ClickHouseBatchWriter {
    ...
    public void persistRecords(List<ClickHouseStruct> records) {
        ...
        try {
            ...
            if (result) {
                ...
                records.forEach(record -> {
                    try {
                        ...
                        log.info("Error marking records as processed" + e);
                    }
            ...
            log.info("Error persisting records to ClickHouse" + e);
        }
    }
